...
const testBackendConnection = async () => {
  ...
  for (const path of paths) {
    try {
      console.log(`嘗試連接 ${path}...`);
      const params = path.includes('reviews') ? { page: 0, size: 1 } : {};
      const response = await axios.get(path, { params, timeout: 5000 });
      
      if (response.status >= 200 && response.status < 300) {
        connectionStatus.value = `✅ 連接成功！路徑: ${path}`;
        console.log(`連接成功! 路徑: ${path}`, response.data);
  ...
  if (!success) {
    connectionStatus.value = '❌ 連接失敗！請確保後端服務正在運行';
    console.error('所有路徑都連接失敗');
  }
};
